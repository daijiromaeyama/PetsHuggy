class ReservationsController < ApplicationController
  ...
  def create
    ...
    if current_user == @listing.user
      ...
      reservationsByme = @listing.reservations.where(user_id: current_user)
  ...
  def setdate
    ...
    listing = Listing.find(params[:listing_id])
    today = Date.today
    reservations = listing.reservations.where("start_date >= ? OR end_date >= ?", today, today) # 今日以降の予約が入っている
  ...
  def is_duplicate(start_date, end_date)
    ...
    check = listing.reservations.where("? < start_date AND end_date < ?", start_date, end_date) # start_date と end_dateがあるかどうか
